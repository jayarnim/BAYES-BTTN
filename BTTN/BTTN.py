...
class Module(nn.Module):
    def __init__(self, n_query, n_key, n_dim, sampler_type="lognormal"):
        ...
        if sampler_type == "weibull":
            self.sampler = APPROX.Weibull(n_query, n_key)
        elif sampler_type == "lognormal":
            self.sampler = APPROX.LogNormal(n_query, n_key)

        self.query = nn.Embedding(n_query, n_dim)
        self.key = nn.Embedding(n_key, n_dim)
    ...
    def forward(self, Q_idx, K_idx, V_idx):
        ...
        context = torch.matmul(attention_weight, V_idx)
